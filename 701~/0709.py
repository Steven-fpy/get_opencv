...
# 픽셀값이 0인 배경으로 부터 거리를 해당 픽셀(대조하는 픽셀) 영역에 표현하는 방법
# 배경으로 부터 멀리 떨어져 있을 수록 높은 픽셀값을 가짐
dist = cv2.distanceTransform(src, distanceType = cv2.DIST_L1, maskSize = 5)
minVal, maxVal, minLoc, maxLoc = cv2.minMaxLoc(dist)
print('src: ', minVal, maxVal, minLoc, maxLoc)

dst = cv2.normalize(dist, None, 0, 255, cv2.NORM_MINMAX, dtype = cv2.CV_8U)
# ret,dst = cv2.threshold(dist, maxVal, 255, cv2.THRESH_BINARY)

# ret, dst2 = cv2.threshold(dist, maxVal - 1, 255, cv2.THRESH_BINARY)
ret, dst2 = cv2.threshold(dist, 25, 255, cv2.THRESH_BINARY)
