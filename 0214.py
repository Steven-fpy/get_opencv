...
class Video(animation.FuncAnimation):
	def __init__(self, device = 0 , fig = None, frames = None,
		...
		super(Video, self).__init__(self.fig, self.updateFrame,
								init_func = self.video_init, frames = frames,
	...
	def updateFrame(self, k):
		retval,self.frame = self.cap.read()
		if retval:
			self.im.set_array(cv2.cvtColor(self.frame,
								cv2.COLOR_BGR2RGB))
		#return self.im
	def close(self):
...
# camera = Video('/home/stvn/pyQT/PYQT_study/OpenCv/data/vtest.avi')
plt.show()
camera.close()

#singleton 싱글톤

#/home/stvn/pyQT/PYQT_study/OpenCv/data